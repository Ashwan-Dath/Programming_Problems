...
public class RotatedSortedArraySearch {
    public static void main(String[] args) {
        ...
        // ex : arr = [12, 24, 54, 98, 0, 4, 6, 9], target = 4
        // Answer = 5 : Because at 5th Index the Target Element is present,

        // ex: arr = [21, 65, 87, 98, 102, 123, 1, 8, 12, 19] target = 102.
        // answer = 4 : Because at 4th index the Target Element is present

        // ex: arr = [4, 8, 21, 65, 87, 98, 102, 123, 1] target = 5.
        // answer = -1 : Returning -1, because target value is not present in the given

        // Explanation the answer should only take O(logn) time only
    ...
    public static int findPivotDuplicateArray(int arr[]) {
        ...
        while (start <= end) {
            ...
            // if the start = mid = end have all the same numbers(duplicates then we have to remove them)
            // ex [6, 8, 10, 6, 0, 1, 6]
            // here start = mid = end, now we have to remove them and check the remaining elements so before removing them
            // we check whether start | end may be the pivot.
            // after removing(changing the start and end to checkt in the array) the array will be like [8, 10, 6, 0] now we find pivot inside this
            if(arr[start] == arr[mid] && arr[mid] == arr[end]){
                // Now we have to skip the duplicates
                // Before we remove these elements we first check whether they are pivot or not

                // we are checking whether the start is pivot or not by comparing it with its next element,
                // If the start element itself is greater than its preceding element then obviously the start will be the highest element in the array.
                ...
                // we are checking whether the end is pivot or not by comparing it with its previous element,
                // If the end element itself is less than its previous element then obviously the end - 1 will be the highest element in the array. Because it has Rotated Array characteristics
                // so we return start.  (Take a sample array and debug it to see the results.)
            ...
            // Here we are checking whether the start is < to mid - or else - is start = mid > end if it applies then we have to move the start to mid + 1 to check the right side of the array
            else if(arr[mid] < arr[start] || (arr[mid] == arr[start] && arr[mid] > arr[end] )){
                start = mid + 1;
            }else{
                // Here we don't need to check whether the end is equal to mid - or else - is start = mid = end if it applies then we have to move the end to mid - 1 to check the Left side of the array
                end = mid - 1;
            }
            // //  if the pivot is present somewhere in between mid & end then we don't need to check the values present from start to mid
            // //  so we are moving our start to mid + 1 so that we can check the pivot in between mid to end.
